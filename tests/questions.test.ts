...
const QUIZ_FIXTURE = `# Study fixture

## Encoding and retrieval

Encoding converts an experience into a memory trace. Retrieval rebuilds that
trace later. Consolidation slowly stabilizes new traces during sleep.

- **Sensory memory**: an ultra-short buffer for raw sights and sounds.
- **Working memory**: the mental workspace that holds a few chunks at a time.
- Long-term memory: the store of facts, events, and skills.

| Cause        | What goes wrong                   |
| ------------ | --------------------------------- |
| Decay        | Unused traces fade with time      |
| Interference | Similar memories compete          |

Q: Why do practice tests beat re-reading?
A: Retrieval strengthens the memory trace — the testing effect.
`;

const material = extractStudyMaterial(QUIZ_FIXTURE);
const examMaterial = extractStudyMaterial(SAMPLE_MARKDOWN);
...
describe('buildQuiz', () => {
  ...
  it('builds a quiz from the Exam 1 deck', () => {
    const quiz = buildQuiz(examMaterial.terms, examMaterial.clozes, { count: 10, seed: 2 });
    expect(quiz.length).toBeGreaterThan(0);
    expect(examMaterial.terms.length).toBeLessThanOrEqual(140);
  });
});
